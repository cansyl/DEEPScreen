...
from dataProcessing import getTopNModels, mirrorImageReturnMatrix, rotateImageReturnMatrix, getSMILEsForAllChEMBL, getModelThresholds, drawPictureandReturnImgMatrix
import sys
IMG_SIZE = 200
# IMG_SIZE = 266
...
def loadModelNew(target, model_fl):
    ...
    for comp_id in comp_id_list:
        ...
        try:
            img_arr = drawPictureandReturnImgMatrix(TEMP_IMG_OUTPUT_PATH, compound_smiles_dict[comp_id], comp_id)
            rotateImageReturnMatrix(test_data, img_arr, np.array([0, 0]), comp_id)
            mirrorImageReturnMatrix(test_data, img_arr, np.array([0, 0]), comp_id)
            test_data.append([np.array(img_arr / 255.0), np.array([0, 0]), comp_id])
        except:
            pass
        test_x = np.array([i[0] for i in test_data]).reshape(-1, IMG_SIZE, IMG_SIZE, 1)
